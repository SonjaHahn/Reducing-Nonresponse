...
#  guides(col=guide_legend(title=NULL))+
  theme_minimal()+
    scale_color_discrete(name = "Instruction", labels = c("motivating", "neutral"))
  #scale_color_ipsum() +
  #theme_ipsum()
plot_length

library(patchwork)
plot_prop + plot_length

ggsave("descr_OEI.pdf", width = 9)

1- mean(data_s1$no_oeq_answer, na.rm = TRUE)
...
int_brms <-readRDS("int_brms1.rds")

summary(int_brms)
sjPlot::tab_model(int_brms, file = "humodel.doc")
