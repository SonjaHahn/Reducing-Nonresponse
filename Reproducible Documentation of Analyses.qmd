...
# 
# tab_model(m1)
# # Die unterscheiden sich, weil standardmäßig der Medien der posterior-distribution berichtet wird?
# 
# # zinb <- read.csv("http://stats.idre.ucla.edu/stat/data/fish.csv")
# # set.seed(123)
# # m1 <- brm(bf(
...
# 
# 
# base_glm <- glmmTMB(answerlength_incl_zero ~ 1 + as.factor(question_id) +
#                      (1 | teacher_id) + (1 | respondent_hash),
#                   data = data_poisson,
#                   ziformula =~ 1 + as.factor(question_id) +
#                    (1 | teacher_id) + (1 | respondent_hash),
#                  family = poisson)
# # Warning: Model convergence problem; extreme or very small eigenvalues detected. See vignette('troubleshooting')
# 
# saveRDS(base_glm,"base_glm.rds")
# 
# base_glm <-readRDS("base_glm.rds")
# 
# summary(base_glm)
# sjPlot::tab_model(base_glm)
...
# uVs_glm <- glmmTMB(answerlength_incl_zero ~ 1 + as.factor(question_id) + 
#                      motiv_cue + as.factor(position) + 
#                      (1|teacher_id / respondent_hash),
#                   data = data_poisson,
#                   ziformula =~ 1 + as.factor(question_id) + 
#                      motiv_cue + as.factor(position)  + 
#                      (1|teacher_id / respondent_hash),
...
# # Warning: Model convergence problem; extreme or very small eigenvalues detected. See vignette('troubleshooting')
# 
# saveRDS(uVs_glm,"uVs_glm.rds")
# 
# uVs_glm <-readRDS("uVs_glm.rds")
# 
# summary(uVs_glm)
# sjPlot::tab_model(uVs_glm)




# int_glm <- glmmTMB(answerlength_incl_zero ~ 1 + as.factor(question_id) + 
#                      motiv_cue * as.factor(position) + 
#                      (1|teacher_id / respondent_hash),
#                   data = data_poisson,
#                   ziformula = ~  1 + as.factor(question_id) + 
...
#                  family = poisson)
# 
# saveRDS(int_glm,"int_glm.rds")

# int_glm <-readRDS("int_glm.rds")
# 
# summary(int_glm)
# sjPlot::tab_model(int_glm)
...
pp_check(base_brms)+
    xlim(0, 100)
```
## pp_check mit einfacheren Modellen

```{r}
set.seed(74791)

null_brms <- brm(bf(answerlength_incl_zero  ~ 1 ,
                               zi ~ 1 ),
                            family = zero_inflated_poisson(),
                            data = data_poisson)
saveRDS(null_brms,"null_brms.rds")

null_brms <-readRDS("null_brms.rds")

summary(null_brms)
pp_check(null_brms) + xlim(0,100)

set.seed(74791)

mini_brms <- brm(bf(answerlength_incl_zero  ~ 1 + as.factor(question_id),
                               zi ~ 1 + as.factor(question_id)),
                            family = zero_inflated_poisson(),
                            data = data_poisson)
saveRDS(mini_brms,"mini_brms.rds")

mini_brms <-readRDS("mini_brms.rds")

summary(mini_brms)
pp_check(mini_brms) + xlim(0,100)
...
## -> ESS soll möglichst groß sein
##   * Diverse Nullmodelle schätzen (siehe direkt darüber) und dann entscheiden
...
```{r}
# Next Steps
#  * Prädiktorvariablen einpflegen (2 uVs, bei Länge und bei zi)
...
# Item als Prädiktor mit Kursleiter und person als clustervariable
